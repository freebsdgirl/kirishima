...
async def memory(parameters: Dict[str, Any]) -> ToolCallResponse:
    """
...
async def _memory_search(parameters: Dict[str, Any]) -> ToolCallResponse:
    ...
    try:
        ...
        if result.get("status") == "ok" and result.get("memories"):
            ...
            return ToolCallResponse(result={"success": True, "data": compact_result})
        else:
        ...
        return ToolCallResponse(result={"success": False, "error": f"HTTP error: {str(e)}"})
    except Exception as e:
        logger.error(f"Error in memory search: {e}")
        return ToolCallResponse(result={"success": False, "error": str(e)})


async def _memory_create(parameters: Dict[str, Any]) -> ToolCallResponse:
    """Create a new memory via the ledger service."""
    try:
        memory_text = parameters.get("memory")
        if not memory_text:
            return ToolCallResponse(result={"success": False, "error": "Memory text is required"})
...
async def _memory_update(parameters: Dict[str, Any]) -> ToolCallResponse:
    """Update an existing memory via the ledger service."""
    try:
        memory_id = parameters.get("memory_id")
        if not memory_id:
            return ToolCallResponse(result={"success": False, "error": "Memory ID is required"})
        ...
        if not update_data:
            return ToolCallResponse(result={"success": False, "error": "At least one field to update is required"})
...
async def _memory_delete(parameters: Dict[str, Any]) -> ToolCallResponse:
...
async def _memory_list(parameters: Dict[str, Any]) -> ToolCallResponse:
...
async def _memory_get(parameters: Dict[str, Any]) -> ToolCallResponse:
