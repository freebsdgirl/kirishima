...
4. Supports comprehensive dry-run mode
"""
...
def _find_similar_topics(
    ...
    try:
        ...
        for i, topic in enumerate(topics):
            ...
            if similar_indices:
                ...
                topic_group = TopicGroup(
                    ...
                    similarity_score=float(np.mean([similarity_matrix[i][j] for j in similar_indices])),
        ...
        # Sort by total memories (largest first) and limit
        topic_groups.sort(key=lambda x: x.total_memories, reverse=True)
        topic_groups = topic_groups[:max_groups]
        
        logger.info(f"Found {len(topic_groups)} similar topic groups")
        return topic_groups
...
@router.post("/memories/_dedup_topic_based")
